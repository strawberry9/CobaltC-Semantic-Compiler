...
- Nominal local structs with fixed-width integer, `bool`, `char`, fixed-size arrays, and nested struct fields; named construction, field reads/writes, whole-struct copies, moves, and replacement, partial field moves and reinitialization, per-field initialization at CFG joins, guarded scalar-field cleanup, and by-value function parameters and returns.
...
The latest recorded validation has **481 passing tests**, exercising **82 of 1,265 rule IDs**. Exercised rules are marked partial; these counts are evidence of tested behavior, not a percentage of language completion. Regenerate the evidence with `python3 tools/coverage_report.py`.
